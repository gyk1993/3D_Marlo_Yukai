...
void xpcosc_InitializeDataMapInfo(RT_MODEL_xpcosc_T *xpcosc_M
  )
{
  /* Set C-API version */
  rtwCAPI_SetVersion(xpcosc_M->DataMapInfo.mmi, 1);

  /* Cache static C-API data into the Real-time Model Data structure */
  rtwCAPI_SetStaticMap(xpcosc_M->DataMapInfo.mmi, &mmiStatic);

  /* Cache static C-API logging data into the Real-time Model Data structure */
  rtwCAPI_SetLoggingStaticMap(xpcosc_M->DataMapInfo.mmi, NULL);

  /* Cache C-API Data Addresses into the Real-Time Model Data structure */
  rtwCAPI_SetDataAddressMap(xpcosc_M->DataMapInfo.mmi, rtDataAddrMap);

  /* Cache C-API Data Run-Time Dimension Buffer Addresses into the Real-Time Model Data structure */
  rtwCAPI_SetVarDimsAddressMap(xpcosc_M->DataMapInfo.mmi, rtVarDimsAddrMap);

  /* Cache C-API rtp Address and size  into the Real-Time Model Data structure */
  xpcosc_M->DataMapInfo.mmi.InstanceMap.rtpAddress = rtmGetDefaultParam(xpcosc_M);
  xpcosc_M->DataMapInfo.mmi.staticMap->rtpSize = sizeof(P_xpcosc_T);

  /* Cache the instance C-API logging pointer */
  rtwCAPI_SetInstanceLoggingInfo(xpcosc_M->DataMapInfo.mmi, NULL);

  /* Set Reference to submodels */
  rtwCAPI_SetChildMMIArray(xpcosc_M->DataMapInfo.mmi, NULL);
  rtwCAPI_SetChildMMIArrayLen(xpcosc_M->DataMapInfo.mmi, 0);
}
