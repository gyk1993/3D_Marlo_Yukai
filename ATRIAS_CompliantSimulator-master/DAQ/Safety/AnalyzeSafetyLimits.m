...
Coords.qsp1 = struct('T', Tqsp1, 'Lims', [-2	5	-200	300	-1	3]);
...
Coords.q3RL = struct('T', Tq3RL, 'Lims', [-25 18 -300 300 -25 18]);
...
for l=1:length(LOG)
    ...
    for k=1:length(LOG(l).violation)
        LOG(l).violation(k) = any(MARLOBasicSafetyLimits(LOG(l).q(k,:).', LOG(l).dq(k,:).'));
