function animateRealTime(q_sol,t_sol, view_angle, make_film_flag,even_sample_fs, foot_adjust_params,foot_geo,ground_terrain_data,leg,avifile_name)
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if nargin<8
        ...
        Z = 0.*Y;
        ground_terrain_data.X = X;
        ground_terrain_data.Y = Y;
        ground_terrain_data.Z = Z;
    end
...
%     for i=1:temp:length(t_even)
        ...
        [pcm p0 p0T pHR p1R p2R p3R p4R pHL p1L p2L p3L p4L] =  ATRIAS3D_Positions_Hip(q_sol);
        ...
        if draw_feet
            [p5L, p6L, p7L, p8L, p9L, p5R, p6R, p7R, p8R, p9R] = ATRIAS3D_Foot_Points(q_sol,foot_adjust_params,foot_geo);
...
%         zlim(ax,[pcm(3)-0.2*nRate-1 pcm(3)+1.8*nRate-1]);
		...
		if view_angle ~= 7
			...
			setview(ax,q_sol(4)*180/pi);
		end
        grid(ax, 'on');
        
        %Add time readout
        time_string = sprintf('time=%0.2f s',t_sol);
