...
#define 	S_FUNCTION_NAME 	sg_IO511_rdhwfifo_s
...
#define FIFOSIZE_ARG                ssGetSFcnParam(S,3)
...
#include "util_xpcimport.c"
    ...
    ssSetOutputPortWidth(S, 0, mxGetPr(FIFOSIZE_ARG)[0] + 1);
...
static void mdlStart(SimStruct *S)
{
    ...
    if ((int_T)mxGetPr(SLOT_ARG)[0]<0) {
        if (rl32eGetPCIInfoExt(vendorid, deviceid, subvendorid, subdeviceid, &pciinfo)) {
            sprintf(msg,"[Read HW Fifo]IO511 board not present");
            ssSetErrorStatus(S,msg);
            return;
        }
    } else {
        ...
        if (rl32eGetPCIInfoAtSlotExt(vendorid, deviceid, subvendorid, subdeviceid, (slot & 0xff) | ((bus & 0xff)<< 8), &pciinfo)) {
            sprintf(msg,"[Read HW Fifo]: IO511 board (at bus %d, slot %d) not present", bus, slot );
            ssSetErrorStatus(S,msg);
            return;
        }
    }
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    ioaddress8 =(volatile uint8_T *) base;   
    
    chan = (uint32_T)mxGetPr(CHANNEL_ARG)[0] - 1;
    ePtrUInt16 = ssGetOutputPortSignal(S,0);
    enable = (uint32_T *)ssGetInputPortSignal(S,0);    
    
    if (enable[0] > 0) {
        
        /* Begining of critical section */
        flags = saveAndDisableInterrupts();  
        count = (ioaddress32[0xD0/4 + chan] >> 16) & 0xFF;
        
        i = 0;
        count = (ioaddress32[0xD0/4 + chan] >> 16) & 0xFF;
        while (count > 0){
            i += 1;
            tmp = ioaddress32[(0x18 + 0x10 * chan)/4];
            ePtrUInt16[i] = tmp;
            //printf("%d: %u\n", i, tmp & 0xFF);      
            count = (ioaddress32[0xD0/4 + chan] >> 16) & 0xFF;
        }       
        //printf("%d values have been read from external RX FIFO.\n", i);
        ePtrUInt16[0] = i;    
        
        ioaddress32[0x64/4] = 0x4 << (0x4 * chan);
