...
static void output1(double *p_output1,const double *x)
{
  ...
  p_output1[33]=-0.1831*(-1.*t11*t15*t7 - 1.*t11*t12*t6*t7 + t10*t12*t15*t8 + t10*t6*t8 + t14*t16*t7*t8);
  p_output1[34]=-0.1831*(-1.*t11*t19*t6 + t11*t13*t3*t7 - 1.*t25*t8 - 1.*t10*t11*t15*t9 - 1.*t6*t7*t8*t9);
  p_output1[35]=-0.1831*(t10*t11*t15*t3 + t3*t6*t7*t8 + t11*t13*t7*t9 - 1.*t11*t6*(-1.*t10*t12*t3 + t14*t9) - 1.*t8*(t13*t14*t3 + t10*t14*t16*t3 - 1.*t12*t15*t3*t7 + t10*t12*t13*t9 + t12*t16*t9));
}
...
void dJ_qALeft_raw(double *p_output1, const double *x)
{
  // Call Subroutines
  output1(p_output1, x);
