...
static void output1(double *p_output1,const double *x)
{
  ...
  t2 = x[3];
  t3 = Cos(t1);
  p_output1[0]=0. + x[0] + 0.58*Sin(t1);
  p_output1[1]=0. + x[1] - 0.58*t3*Sin(t2);
  p_output1[2]=0. + 0.58*t3*Cos(t2) + x[2];
}
...
void p_TorsoTop_raw(double *p_output1, const double *x)
{
  // Call Subroutines
  output1(p_output1, x);
