classdef RobotAnimator < handle
    ...
    methods
        function obj = RobotAnimator(t, q, positions_function, FactorTime, varargin)
            ...
            % Initialization
            [ pH, pT, pRK, pR, pLK, pL ] = obj.positions_function(obj.step.q(1,:));
...
%             obj.ground = surf(terrain.Tx,terrain.Ty,terrain.Tz);
            ...
            obj.pH_RKnee = line([pH(1) pRK(1)],[pH(2) pRK(2)]);
            obj.pH_LKnee = line([pH(1) pLK(1)],[pH(2) pLK(2)]);
            obj.pRKnee_RFoot = line([pRK(1) pR(1)],[pRK(2) pR(2)]);
            obj.pLKnee_LFoot = line([pLK(1) pL(1)],[pLK(2) pL(2)]);
            obj.pHT = line([pH(1) pT(1)],[pH(2) pT(2)]);
        ...
        function flag = Animate(obj)
            ...
            for i = obj.starting_index:n
                ...
                for j=1:length(obj.axisCells)
                    % Update Positions
                    [ pH, pT, pRK, pR, pLK, pL ] = obj.positions_function(obj.step.q(i,:));
                    axis(obj.axisCells{j},[pH(1)-1, pH(1)+1, pH(2)-1, pH(2)+1]);
                    % 					view(obj.views{j})
                    
                                  
                    set(obj.pH_RKnee,'XData',[pH(1) pRK(1)],'YData',[pH(2) pRK(2)]);
                    set(obj.pH_LKnee,'XData',[pH(1) pLK(1)],'YData',[pH(2) pLK(2)]);
                    set(obj.pRKnee_RFoot,'XData',[pRK(1) pR(1)],'YData',[pRK(2) pR(2)]);
                    set(obj.pLKnee_LFoot,'XData',[pLK(1) pL(1)],'YData',[pLK(2) pL(2)]);
                    set(obj.pHT,'XData',[pH(1) pT(1)],'YData',[pH(2) pT(2)]);
