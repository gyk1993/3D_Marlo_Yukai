...
#define 	S_FUNCTION_NAME 	sg_IO511_txintenable_s
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    // This driver only changes the enable value of the interrupt sources at the IO511 level
    // PLX and USC interrupt registers are always enabled as soon as the model is initialized.  
    
    chan = (uint32_T)mxGetPr(CHANNEL_ARG)[0] - 1;    
    ePtrBool = (boolean_T *)ssGetInputPortSignal(S, 0);
    
    // If data is present inside the software FIFO we allow the board to request data and generates interrupts to populate its HW FIFO.    
    if (ePtrBool[0]){    
        ioaddress32[(0x18 + chan * 0x10)/4] = 0x0;
        ioaddress32[(0x18 + chan * 0x10)/4] = 0x0;
        icr = ioaddress32[0x0060 / 4];
        icr |= (0x2 << (0x4 * chan));
        ioaddress32[0x0060 / 4] = icr;        
    }
