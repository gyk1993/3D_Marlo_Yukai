...
/* fiforeadbinhdr.c - xPC Target, non-inlined S-function to perform the read
 * side of a fifo read/write pair given several binary headers and message
 * lengths */
...
#define         S_FUNCTION_NAME         fiforeadbinhdr2
...
printf("start: rptr = %d, wptr = %d\n", *rptr, *wptr );
...

    {
        ...
        for( i = 0 ; i < fifocount ; i++ )
        {
            ...
            // BGB
            // Stop looking if we found one good packet; this prevents the
            // FIFO from dropping all but the last good packet.
            if (hdrfound == 1)
                break;
            // END BGB
