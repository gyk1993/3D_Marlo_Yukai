...
%         'stepLength',3,RightStance.nNode,{{'q'}},-5e-4,5e-4,extra);
    ...
    % Periodicity
    selected = ones(1,DOF); selected([1:2]) = 0;
...
%         'stepLength',3,LeftStance.nNode,{{'q'}},-5e-4,5e-4,extra);
    ...
    for i=1:obj.nDomain
        ...
        domain = addConstraint(domain,'Nonlinear-Inequality',...
            'symmetry',3,[1, domain.nNode],{{'q'}},deg2rad(-0.01),deg2rad(0.01));
        
        % Knee Angles
        domain = addConstraint(domain,'Nonlinear-Inequality',...
            'kneeAngles',2,1:domain.nNode,{{'q'}},deg2rad(55),deg2rad(90));
        
        % Torso
        domain = addConstraint(domain,'Nonlinear-Inequality',...
            'torsoPitch',1,1:domain.nNode,{{'q'}},deg2rad(-0.1),deg2rad(0.1));
        domain = addConstraint(domain,'Nonlinear-Inequality',...
            'torsoRoll',1,1:domain.nNode,{{'q'}},deg2rad(-0.1),deg2rad(0.1));
        
        % Hip
        domain = addConstraint(domain,'Nonlinear-Inequality',...
            'hipAngles',2,1:domain.nNode,{{'q'}},deg2rad(-3),deg2rad(3));
        ...
        % Swing Foot Velocity at Impact
        %velBoundsLow = [0, 0, 0];
        %velBoundsHigh = [0, 0, 0];
        %domain = addConstraint(domain,'Nonlinear-Inequality',...
        %    'swingFootClearance',1,domain.nNode,{{'q','dq'}},velBoundsLow, velBoundsHigh);
