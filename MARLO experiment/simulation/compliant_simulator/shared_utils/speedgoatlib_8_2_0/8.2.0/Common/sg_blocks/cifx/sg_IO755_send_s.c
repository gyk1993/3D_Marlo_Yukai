...
#define OUTLENGTH_ARG       ssGetSFcnParam(S,3)
#define PORTTYPE_ARG        ssGetSFcnParam(S,4)
...
static void mdlInitializeSizes(SimStruct *S)
{
    ...
    out_length = (int)mxGetPr(OUTLENGTH_ARG)[0];
    
    port_type = (int)mxGetPr(PORTTYPE_ARG)[0];
    
    if (port_type == 1) // n ports
    {
...
static void mdlOutputs(SimStruct *S, int_T tid)
{
    ...
    int out_length = (int)mxGetPr(OUTLENGTH_ARG)[0];
    int out_address = (int)mxGetPr(OUTADDRESS_ARG)[0];
    ...
    // send data
    sg_cifx_write(cardId, out_address, out_length, data);
