...
function control_gui_OpeningFcn(hObject, eventdata, handles, varargin)
	...
	handles.FigureObject = hObject;
	
	% Store joystick interface to the gui
    
	handles.joystick = VRJoystickWrapper(1);
%     [axes, buttons, pov, buttonsRiseEdge, povRiseEdge] = handles.joystick.Tick();
	handles.timer = timer('TimerFcn', @JoystickTimerFcn, ...
		'ExecutionMode', 'fixedDelay', ...
		'Period', 0.1, ...
		'UserData', getParentFigure(hObject));
	handles.axis_sum = zeros(size(axes));
	handles.axis_count = 0;
    
	handles.ps3Axes = zeros(1,5);
	handles.ps3Buttons = zeros(1,11);
    handles.ps3Pov = -1;
    handles.print_count = 0;
...
function main_Callback(hObject, eventdata, handles)
	...
	switch get(hObject, 'Tag')
		...
		case {'sat_zero_fast'}
			...
			pause(0.1);
			setparamsafe(handles.tg, handles.paramid.MedullaCommand, 0); % 0=Init
		...
		case {'fp_enable_transition'}
			xpc_enable_transition_fcn(handles);
		...
		case {'TorsoCOMRollOffset', 'TorsoCOMPitchOffset'}
			TorsoCOMOffsets = ReadTorsoCOMOffsets(handles);
			WriteTorsoCOMOffsets(handles, TorsoCOMOffsets);
			UpdateTorsoCOMOffsets(handles, TorsoCOMOffsets);
		...
		case {'xpc_load'}
			xpc_load_fcn(handles);
			
		case {'xpc_start'}
			xpc_start_fcn(handles);
			
		case {'xpc_stop'}
			xpc_stop_fcn(handles);
			
		case {'xpc_logdata'}
			xpc_logdata_fcn(handles);
	end
...
% 	XY_Offsets = [handles.ps3Axes, handles.ps3Buttons];
	setparamsafe(handles.tg, handles.paramid.ps3Axes, handles.ps3Axes);
	setparamsafe(handles.tg, handles.paramid.ps3Buttons, handles.ps3Buttons);
	setparamsafe(handles.tg, handles.paramid.ps3Pov, handles.ps3Pov);
	UseIMU = get(handles.UseIMU,'Value');
	%    setparamsafe(handles.tg, handles.paramid.UseIMU, UseIMU);
	%{
    imu_up_compensation = str2double(get(handles.imu_up_compensation,'String'));
...
function xpc_load_fcn(handles)
	hObject = handles.FigureObject;
	if ~checkstate(handles,'paramid','q')
		try
			handles = init_xpc(handles,false);
			guidata(hObject, handles);
...
function xpc_start_fcn(handles)
	hObject = handles.FigureObject;
	if ~checkstate(handles,'paramid','q')
		try
			handles = init_xpc(handles,false);
...
function xpc_medulla_run_fcn(handles)
	hObject = handles.FigureObject;
	if ~checkstate(handles,'paramid','q')
		try
			handles = init_xpc(handles,false);
...
function xpc_logdata_fcn(handles)
	hObject = handles.FigureObject;
	if ~checkstate(handles,'paramid','q')
		try
			handles = init_xpc(handles,false);
	...
	[time, output, status, log, datalogfilename] = getExperimentData(handles.tg, handles.figure1);
	assignin('base','time',time);
	assignin('base','output',output);
	assignin('base','status',status);
	assignin('base','log',log);
    assignin('base','datalogfilename',datalogfilename);
	set(handles.xpc_logdata, 'Enable', 'on')
	
function xpc_enable_transition_fcn(handles)
	hObject = handles.FigureObject;
	if ~checkstate(handles,'paramid','q')
		try
			handles = init_xpc(handles,false);
...
function UpdateTorsoCOMOffsets(handles, TorsoCOMOffsets)
	hObject = handles.FigureObject;
	if ~checkstate(handles,'paramid','q')
		try
			handles = init_xpc(handles,false);
...
function TorsoCOMOffsets = ReadTorsoCOMOffsets(handles)
	TorsoCOMOffsets = str2double(get([handles.TorsoCOMRollOffset; handles.TorsoCOMPitchOffset],'String'));
	
function WriteTorsoCOMOffsets(handles, TorsoCOMOffsets)
	TorsoCOMRollOffsets_string = sprintf('%0.1f', TorsoCOMOffsets(1));
	TorsoCOMPitchOffsets_string = sprintf('%0.1f', TorsoCOMOffsets(2));
	set(handles.TorsoCOMRollOffset, 'String', TorsoCOMRollOffsets_string);
	set(handles.TorsoCOMPitchOffset, 'String', TorsoCOMPitchOffsets_string);
	
function XY_Offsets = ReadXY_Offsets(handles)
	XY_Offsets = [handles.ps3Axes, handles.ps3Buttons];
	
function WriteXY_Offsets(handles,axes,buttons,pov)
    buttons = double(buttons);
	ps3Axes = axes;
	ps3Buttons = buttons;
	ps3Pov = pov;
    
	setparam(handles.tg, handles.paramid.ps3Axes, ps3Axes);
	setparam(handles.tg, handles.paramid.ps3Buttons, ps3Buttons);
	setparam(handles.tg, handles.paramid.ps3Pov, ps3Pov);
    
%     fprintf('xy_offset = (%.4f, %.4f)\n', ps3Axes, ps3Buttons);

function AxesDoSomething(axes, hObject, print)
	handles = guidata(hObject);
	
	dx = -axes(2) * 0.4;
	dy = axes(1) * 0.2;
	
	setparam(handles.tg, handles.paramid.dx_cmd, dx);
	setparam(handles.tg, handles.paramid.dy_cmd, dy);
    
%     if print
%         fprintf('dx,dy = [%.3f, %.3f]\n', dx, dy);
%     end
	
function JoystickTimerFcn(timerObject, ~)
	hObject = timerObject.UserData;
	handles = guidata(hObject);
	...
	if isempty(handles.axis_sum)
		handles.axis_sum = zeros(size(axes));
	end
		
	handles.axis_count = handles.axis_count + 1;
	handles.axis_sum = handles.axis_sum + axes;
	
% 	COUNT_MAX = 10;	
% 	if handles.axis_count == COUNT_MAX
% 		axes_filtered = handles.axis_sum ./ COUNT_MAX;
% 		
% 		handles.axis_sum = zeros(size(axes));
% 		handles.axis_count = 0;
% 		
%         handles.print_count = handles.print_count + 1;
%         if handles.print_count >= 20
%             AxesDoSomething(axes_filtered, hObject, true);
%             handles.print_count = 0;
%         else
%             AxesDoSomething(axes_filtered, hObject, false);
%         end
% 	end	
	
        handles.ps3Axes = axes;
        handles.ps3Buttons = buttons;
		handles.ps3Pov = pov;
%         XY_Offsets = ReadXY_Offsets(handles);
		WriteXY_Offsets(handles, axes,buttons,pov);
