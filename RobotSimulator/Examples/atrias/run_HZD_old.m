...
optimization = load('mat\walkInPlace_old');
N = length(optimization.outputs{1}.t);
M = 5;

% Construct Desired q trajectory
HAlpha_q_RightStance_old = bezfit(linspace(0,1,N), optimization.outputs{1}.q', M);
HAlpha_q_LeftStance_old =  bezfit(linspace(0,1,N), optimization.outputs{2}.q', M);

HAlpha_q_RightStance = zeros(16,6);
HAlpha_q_RightStance([1:6,7:9,12:14],:) = HAlpha_q_RightStance_old([1:3,5,4,6,9,7:8,12,10:11],:); 
HAlpha_q_RightStance(12,:) = -HAlpha_q_RightStance(12,:);
HAlpha_q_RightStance([10,11,15,16],:) = HAlpha_q_RightStance([9,8,14,13],:) - HAlpha_q_RightStance([8,9,13,14],:);

HAlpha_q_LeftStance = zeros(16,6);
HAlpha_q_LeftStance([1:6,7:9,12:14],:) = HAlpha_q_LeftStance_old([1:3,5,4,6,9,7:8,12,10:11],:); 
HAlpha_q_LeftStance(12,:) = -HAlpha_q_LeftStance(12,:);
HAlpha_q_LeftStance([10,11,15,16],:) = HAlpha_q_LeftStance([9,8,14,13],:) - HAlpha_q_LeftStance([8,9,13,14],:);

% Construct Desired dq trajectory
HAlpha_dq_RightStance_old = bezfit(linspace(0,1,N), optimization.outputs{1}.dq', M);
HAlpha_dq_LeftStance_old =  bezfit(linspace(0,1,N), optimization.outputs{2}.dq', M);

HAlpha_dq_RightStance = zeros(16,6);
HAlpha_dq_RightStance([1:6,7:9,12:14],:) = HAlpha_dq_RightStance_old([1:3,5,4,6,9,7:8,12,10:11],:); 
HAlpha_dq_RightStance(12,:) = -HAlpha_dq_RightStance(12,:);
HAlpha_dq_RightStance([10,11,15,16],:) = HAlpha_dq_RightStance([9,8,14,13],:) - HAlpha_q_RightStance([8,9,13,14],:);

HAlpha_dq_LeftStance = zeros(16,6);
HAlpha_dq_LeftStance([1:6,7:9,12:14],:) = HAlpha_dq_LeftStance_old([1:3,5,4,6,9,7:8,12,10:11],:); 
HAlpha_dq_LeftStance(12,:) = -HAlpha_dq_LeftStance(12,:);
HAlpha_dq_LeftStance([10,11,15,16],:) = HAlpha_dq_LeftStance([9,8,14,13],:) - HAlpha_q_LeftStance([8,9,13,14],:);
...
%% Run Simulator
q0_old = optimization.outputs{1}.q(1,:)';
dq0_old = optimization.outputs{1}.dq(1,:)';

% Change to new coordinates
q0 = zeros(16,1);
q0([1:6,7:9,12:14]) = q0_old([1:3,5,4,6,9,7:8,12,10:11]); 
q0(12) = -q0(12);
q0([10,11,15,16]) = q0([9,8,14,13]) - q0([8,9,13,14]);

dq0 = zeros(16,1); 
dq0([1:6,7:9,12:14]) = dq0_old([1:3,5,4,6,9,7:8,12,10:11]);
dq0(12) = -dq0(12);
dq0([10,11,15,16]) = dq0([9,8,14,13]) - dq0([8,9,13,14]);

stanceFootPos = p_RightFoot(q0);
q0(3) = q0(3) - stanceFootPos(3);
